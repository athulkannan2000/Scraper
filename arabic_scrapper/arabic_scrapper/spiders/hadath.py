...
site_list,catagory,main_category,sub_category,platform,media_type,urgency = load_dataset_lists("hadath")
now = datetime.now()
...
class HadathSpider(scrapy.Spider):
    ...
    def details_scrapper(self,response):
        ...
        title = response.xpath('//div[@class="innerContent "]/h1[@class="newsTitle marginTop20"]/text()').extract_first()
        hadath["title"] = title.strip()
