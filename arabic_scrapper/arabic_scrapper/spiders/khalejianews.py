...
site_list,catagory,main_category,sub_category,platform,media_type,urgency = load_dataset_lists("khalejianews")
now = datetime.now()
...
class KhalejianewsSpider(scrapy.Spider):
    ...
    def details_scrapper(self,response):
        ...
        contents=response.xpath('//*[@class="article-body"]//p/span//strong/text()').extract()+response.xpath('//*[@class="article-body"]//p//strong/text()').extract()+response.xpath('//*[@class="article-body"]//p/text()').extract()
